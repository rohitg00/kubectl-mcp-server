...
class MCPServer:
    ...
    def setup_tools(self):
                ...
                for quota in quotas:
                    ...
                    for resource_name, hard_val in hard.items():
                        ...
                        resources.append({
                            ...
                            "hard": hard_val,
                            "used": used_val,
                            "available": self._calculate_available(hard_val, used_val)
                        })
        ...
        self._calculate_available = _calculate_available
                ...
                for pod in pods:
                    ...
                    for container in pod.spec.containers:
                        if container.resources and container.resources.requests:
                            ...
                            pod_cpu += self._parse_cpu(cpu)
                            pod_memory += self._parse_memory(memory)
        ...
        self._parse_cpu = _parse_cpu
        self._parse_memory = _parse_memory
        ...
        def get_overprovisioned_resources(
            ...
            try:
                ...
                for pod in pods:
                    ...
                    for container in pod.spec.containers:
                        if container.resources and container.resources.requests:
                            total_cpu_req += self._parse_cpu(container.resources.requests.get("cpu", "0"))
                            total_mem_req += self._parse_memory(container.resources.requests.get("memory", "0"))

                    if total_cpu_req == 0 and total_mem_req == 0:
                        continue

                    cpu_used = self._parse_cpu(usage["cpu"])
                    mem_used = self._parse_memory(usage["memory"])
            ...
            try:
                ...
                for line in result.stdout.strip().split("\n"):
                    ...
                    if resource_type == "nodes" and len(parts) >= 5:
                        cpu_val = self._parse_cpu(parts[1])
                        mem_bytes = self._parse_memory(parts[3])
                        total_cpu += cpu_val
                        ...
                        if namespace:
                            cpu_val = self._parse_cpu(parts[1])
                            mem_bytes = self._parse_memory(parts[2])
                            metrics.append({
                                "namespace": namespace,
                                "pod": parts[0],
                                "cpuUsage": parts[1],
                            ...
                            cpu_val = self._parse_cpu(parts[2])
                            mem_bytes = self._parse_memory(parts[3])
                            metrics.append({
                                "namespace": parts[0],
                                "pod": parts[1],
                ...
                for deploy in deployments:
                    ...
                    for container_spec in deploy.spec.template.spec.containers:
                        ...
                        if container_spec.resources and container_spec.resources.requests:
                            current_cpu = self._parse_cpu(container_spec.resources.requests.get("cpu", "0"))
                            current_mem = self._parse_memory(container_spec.resources.requests.get("memory", "0"))
